-- =====================================================
-- MIGRATION: MODO DEMO - Empresa Demo + Coluna is_demo
-- =====================================================

-- PARTE 1: Adicionar coluna is_demo em companies
ALTER TABLE public.companies 
ADD COLUMN IF NOT EXISTS is_demo boolean NOT NULL DEFAULT false;

-- Adicionar comentário descritivo
COMMENT ON COLUMN public.companies.is_demo IS 'Indica se a empresa é uma empresa de demonstração';

-- PARTE 2: Criar a empresa DEMO e vincular admin
DO $$
DECLARE
  v_admin_user_id uuid := '5a564e5c-dbfd-43d9-bc63-4d5080d38609';
  v_demo_company_id uuid;
  v_admin_role_id uuid;
BEGIN
  -- Verificar se já existe empresa DEMO
  ...
  IF v_demo_company_id IS NULL THEN
    INSERT INTO public.companies (name, cnpj, status, is_demo)
    VALUES ('SALLUS DEMO', NULL, 'active', true)
  ...
  -- Criar vínculo do admin com empresa DEMO
  IF v_admin_role_id IS NOT NULL THEN
    INSERT INTO public.user_company_roles (user_id, company_id, role_id, is_primary, is_active)
    VALUES (v_admin_user_id, v_demo_company_id, v_admin_role_id, false, true)
    ON CONFLICT (user_id, company_id) DO UPDATE SET
      role_id = EXCLUDED.role_id,
      is_active = true;
  END IF;
  
  -- Criar configurações padrão para a empresa DEMO
  INSERT INTO public.company_settings (company_id, segment)
  VALUES (v_demo_company_id, 'saude')
  ON CONFLICT (company_id) DO NOTHING;
END $$;

-- PARTE 3: Função segura para reset da empresa DEMO
...
BEGIN
  ...
  INSERT INTO public.productions (company_id, unit_id, payer_type, convenio, production_type, description, total_value, quantity, production_date, status, created_by) VALUES
    (v_demo_company_id, 'UNIDADE_A', 'PARTICULAR', NULL, 'CONSULTA', 'Consulta clínica - Dr. Demo', 350.00, 1, CURRENT_DATE - 15, 'FATURADO', v_user_id),
    (v_demo_company_id, 'UNIDADE_A', 'CONVENIO', 'IPASGO', 'EXAME', 'Hemograma completo', 85.00, 2, CURRENT_DATE - 12, 'PRODUZIDO', v_user_id),
    (v_demo_company_id, 'UNIDADE_B', 'CONVENIO', 'UNIMED', 'CONSULTA', 'Retorno paciente convênio', 180.00, 1, CURRENT_DATE - 10, 'RECEBIDO', v_user_id),
    (v_demo_company_id, 'UNIDADE_A', 'PARTICULAR', NULL, 'PROCEDIMENTO', 'Pequeno procedimento ambulatorial', 650.00, 1, CURRENT_DATE - 5, 'FATURADO', v_user_id),
    (v_demo_company_id, 'UNIDADE_B', 'CONVENIO', 'BRADESCO', 'EXAME', 'Ultrassonografia abdominal', 220.00, 1, CURRENT_DATE - 3, 'PRODUZIDO', v_user_id),
    (v_demo_company_id, 'UNIDADE_A', 'PARTICULAR', NULL, 'CONSULTA', 'Primeira consulta - Novo paciente', 400.00, 1, CURRENT_DATE - 1, 'PRODUZIDO', v_user_id);
  
  -- 6.3 Contas a receber (4 registros)
  INSERT INTO public.receivables (company_id, unit_id, payer_name, payer_type, billed_amount, expected_receipt_date, status, billing_date, created_by) VALUES
    (v_demo_company_id, 'UNIDADE_A', 'IPASGO', 'CONVENIO', 4500.00, CURRENT_DATE + 30, 'FATURADO', CURRENT_DATE - 15, v_user_id),
    (v_demo_company_id, 'UNIDADE_B', 'UNIMED', 'CONVENIO', 3200.00, CURRENT_DATE + 45, 'FATURADO', CURRENT_DATE - 10, v_user_id),
    (v_demo_company_id, 'UNIDADE_A', 'BRADESCO', 'CONVENIO', 1850.00, CURRENT_DATE + 60, 'FATURADO', CURRENT_DATE - 5, v_user_id),
    (v_demo_company_id, 'UNIDADE_B', 'GEAP', 'CONVENIO', 2100.00, CURRENT_DATE - 10, 'RECEBIDO', CURRENT_DATE - 40, v_user_id);
...
-- PARTE 4: Função para verificar se empresa é DEMO (útil para o front)
